...
A slightly expanded suite declaration is needed for this, as shown below. You can declare variables in the suite which can be accessed in its tests (below). The name passed to @END_SUITE_AS@ becomes the name that you use to access the suite variables.
...
TEST(First test) {
  assert( suite.thing->ReadyToBeTested(), SHOULD(be re-initialized for each test));
}
TEST(Second test) {
  assert( suite.thing->ReadyToBeTested(), SHOULD(be re-initialized for each test));
}
...
You can also run code to setup and teardown the complete suite. This is only rarely useful - perhaps to make a connection to a remote machine.

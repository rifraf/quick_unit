...
@assert_equal(<expected>, <actual>[, <message>])@ : fails the test unless @<expected> == <actual>@. @assert_equal@ uses @==@ to do the match, so custom types and classes can be compared if @==@ is supported. For reporting purposes, they must also be able to emit to a std::ostream.

h2. assert_not_equal

@assert_not_equal(<expected>, <actual>[, <message>])@ : fails the test unless @<expected> != <actual>@. @assert_equal@ uses @!=@ to do the match, so the same rules as above apply to custom types.
